...
tt = State(label='tt', energy=1.0, multiplicity=1, size=2, connected_distance=8)
t1 = State(label='t1', energy=1.0, multiplicity=3, size=1)
...
trajectories = calculate_kmc(system,
                             num_trajectories=0,    # number of trajectories that will be simulated
                             max_steps=10000,         # maximum number of steps for trajectory allowed
                             silent=False)


#store_trajectory_list(trajectories, 'singlet_fission.h5')
trajectories = load_trajectory_list('singlet_fission_2.h5')
